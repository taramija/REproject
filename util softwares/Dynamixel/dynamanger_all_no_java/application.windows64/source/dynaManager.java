...
public class dynaManager extends PApplet {
 ...
 *
 ************************************/
  public void drawKnob(float rotation, int limits, int indicator)
  {
    pushMatrix();
    ...
    if(limits ==1)
    {
      arc(45, 45, 75, 75, 1.047f, 2.094f);//draw arc to hide section of middle circle
    }
    //not limited, draw small arc
    else
    {
      arc(45, 45, 75, 75, HALF_PI-.05f, HALF_PI+.05f);//draw arc to hide section of middle circle
...
public byte[] getDynaPacket(int id, int regStart, int length)
{
  ...
  if(inByte == 0xff)
  {
    ...
    {
      localChecksum = PApplet.parseByte(~((responseArray[2] +responseArray[3]+ responseArray[4] + responseArray[5]  ) % 256)); //compute checksum !(ID+length+instruction+parameter1+parameter2%256)
      checksumFromDyna = PApplet.parseByte(responseArray[6]);   
   }
    
    else if(length ==2)//if we have 2 packets, add in the extra parameter to the checksum
    {
      localChecksum = PApplet.parseByte(~((responseArray[2] +responseArray[3]+ responseArray[4] + responseArray[5] + responseArray[6] ) % 256)); //compute checksum !(ID+length+instruction+parameter1+parameter2%256)
      checksumFromDyna = PApplet.parseByte(responseArray[7]);   
   ...
   //fill responseParameter array from the response array (i.e. remove the leading bytes, just retrieve the requested data   
    for(int k = 0; k< length; k++)
    {
     ...
     responseParameter[k] = PApplet.parseByte(responseArray[5+k]);
...
public byte[] intToBytes(int convertInt)
{
  ...
  returnBytes[0] =PApplet.parseByte(convertInt & mask);
  returnBytes[1] =PApplet.parseByte((convertInt>>8) & mask);
  return(returnBytes);
...
public byte[] readDynaPacket(int numberOfRegisters)
{
  ...
  for(int i =0; i < 6+numberOfRegisters;i++)    // If data is available in the serial port, continute
  {
    ...
    {
      inByte = PApplet.parseByte(sPort.readChar());
 ...
 *
 ************************************/
public void sendDynaPacket(byte id, byte instruction, byte[] parameters)
{
  byte checksum = 0;
  int numberOfParameters = parameters.length;  //find out how many parameters to send out
  byte length = PApplet.parseByte(2+numberOfParameters);    //packet length = 2 + number of parameters 
  sPort.clear();
  //add up the values of any parameters for the checksum
  for(int i = 0; i < numberOfParameters; i++)
  {
    checksum = PApplet.parseByte(checksum + parameters[i]);
  }
  //do the rest of the checksum math checksum 
  //!(ID+length+instruction+parameters%256)  
  checksum = PApplet.parseByte(~((id + length + instruction + checksum) % 256)); 
  ...
  if(debug ==1)
  {
    ...
    print("");
    print(hex(PApplet.parseByte(255)));
    print("-");
    print(hex(PApplet.parseByte(255)));
    print("-");
    print(hex(id));
 ...
 *  
 ************************************/
public void sendDynaPacket(int id, int instruction, int parameters)
{
  sendDynaPacket(PApplet.parseByte(id), PApplet.parseByte(instruction), PApplet.parseByte(parameters));
 ...
 *
 ************************************/
 public int writeDynaReg(byte servoId, byte[] parameters)
{
  ...
  sendDynaPacket(servoId, PApplet.parseByte(3), parameters); //send a dynamixel packet to the id# servoid with parameters. '3' is the 'WRITE' instruction
  response = readDynaPacket(numberOfRegisters);//get response of write to check for errors
  
  //check headers
  if(response[0] == PApplet.parseByte(255) && response[1] == PApplet.parseByte(255))
  {
    //check if id returned is the same as the one sent out
    if(response[2] == servoId)
    {
      localChecksum = PApplet.parseByte(~((response[2]+response[3]+response[4]) % 256)); //calculate checksum locally
 ...
 *
 ************************************/
public int writeDynaReg(int servoId, byte[] parameters)
{
  if(debug ==1){println ("writeDynaReg int version-");}
  return(writeDynaReg(PApplet.parseByte(servoId),parameters));
}//end writeDynaReg
 ...
 *
 ************************************/
 public byte[] readDynaReg(byte servoId, byte[] parameters)
{
  ...
  sendDynaPacket(servoId, PApplet.parseByte(2), parameters); //send a dynamixel packet to the id# servoid with parameters. '2' is the 'READ' instruction
  response = readDynaPacket(numberOfRegisters);//get response of write to check for errors
  
  //check headers
  if(response[0] == PApplet.parseByte(255) && response[0] == PApplet.parseByte(255))
  {
    //check if id returned is the same as the one sent out
    if(response[2] == servoId)
    {
      ...
      {
        localChecksum = PApplet.parseByte(localChecksum + response[i+5]); //offset by 5 since the parameters start at 5
        registerData[i] = response[i+5];//make use of this loop to also move register data to the return array
      }      
      localChecksum = PApplet.parseByte(~((response[2]+response[3]+response[4] +localChecksum) % 256)); //do the rest of the math to calculate the checksum locally
 ...
 *
 ************************************/
public byte[] readDynaReg(int servoId, byte[] parameters)
{
  if(debug ==1){println ("readDynaReg int version-");}
  return(readDynaReg(PApplet.parseByte(servoId),parameters));
}//end readDynaReg


public int setServoBaud(int servoId, int servoBaud)
{
    byte[] parameters = {4, PApplet.parseByte(servoBaud)};
    
    return(writeDynaReg(servoId, parameters));
}

public int setServoId(int currentServoId, int newServoId)
{

   byte[] parameters = {3, PApplet.parseByte(newServoId)};
...
public int setGoalPositionBytes(int servoId, byte[]goalPosition)
{
  byte[] parameters = {PApplet.parseByte(30), goalPosition[0], goalPosition[1]};//the goal position is 30, so create a paramater array that starts with 30 and ends with the goal position data 
  writeDynaReg(servoId, parameters);
  return(1);
}
public int setGoalPositionDegree(int servoId, int goalPositionDeg)
{
  
  return(1);
}
public int setGoalPositionInt(int servoId, int goalPositionInt)
{
...
public int setTorqueOn(int servoId)
{
  
  byte[] onVal = {PApplet.parseByte(1)};
  //writeDynaReg(servoId, 24,onVal);
  return(1);

}
public int setTorqueOff(int servoId)
{
  
  byte[] offVal = {PApplet.parseByte(0)};
  //writeDynaReg(servoId, 24,offVal);
  return(1);

}

public int setLedOn(int servoId)
{
...
public int setLedOff(int servoId)
{
...
public int checkId(int servoId)
{
  byte[] returnedId = {0};
  byte[] parameters = {3, 1};
  ...
  if(returnedId != null)
  {
    return(PApplet.parseInt(returnedId[0]));
  }
  else
  {
...
public void servoHeartbeat(int servoId)
{
 ...
 *
 ************************************/
public int getDynaResolution(int servoId)
{
...
public String getDynaModelName(int servoId)
{
...
public int setArbotixBaud(int baud)
{
  byte[] parameters = {PApplet.parseByte(4), 0};//baud is at register 4, placeholder 0 for baud
  byte id = PApplet.parseByte(0xfd);//253 = id to set Arbotix parameters
...
public int pingArbotix()
{

 
  byte[] parameters = {PApplet.parseByte(3), 1};//id is at register 4, length 1 
  byte id = PApplet.parseByte(0xfd);//253 = id to set Arbotix parameters
 ...
 // long startReadingTime = millis();//time that the program started looking for data
  ...
  {
      sendDynaPacket(id, PApplet.parseByte(2), parameters); //send a dynamixel packet to the id# servoid with parameters. '2' is the 'READ' instruction
      response = readDynaPacket(0);//get response to see if arbotix is present
        //check headers
      if(response[0] == PApplet.parseByte(255) && response[1] == PApplet.parseByte(255) && response[2] == PApplet.parseByte(253) && response[3] == PApplet.parseByte(3) && response[4] == PApplet.parseByte(0) && response[5] == PApplet.parseByte(253))
      {
        return(1);
      }
...
return(0);
  ...
  */
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "dynaManager" };
    if (passedArgs != null) {
